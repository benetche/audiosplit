    ...
    <section className="flex flex-col gap-6">
      ...
      {entries === null ? (
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {entries.map((entry) => (
            <LibraryCard key={entry.id} entry={entry} onChanged={refresh} locale={language} />
          ))}
        </div>
      )}
    </section>
...
function LibraryCard({ entry, onChanged, locale }: LibraryCardProps) {
  const { t } = useI18n();
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mono mt-0.5 text-[11px] text-text-muted">
            {new Date(entry.createdAt).toLocaleString(locale)} · {t("library.stemsCount", { count: entry.stems.length })}
          </p>
        </div>
